function C=SetSMARTS295(varargin)
% C= SetSMARTS295(Name/Value pairs)
%Creates structure for input file and then uses struct2SMARTSinputto print
%the input file smarts295.inp.txt, which can be run with smarts295.exe or
%smarts295bat.exe.
%Input options are described in detail in the SMARTS manual
%smarts295-users-manual-pc.pdf, in the Documentation folder of the SMARTS_295_PC root folder.
...
% 'ALTIT' - surface altitude in km, default 0, ISPR = 1 or 2
% 'HEIGHT' - height above surface altitude, km, default 0, ISPR = 1 or 2
...
% 'ISPCTR' - default 0, see documentation for options -1 to 8
%CARD8 aerosol model
...
%   'SOLARC' solar constant default 1367 W/m^2
%CARD12 - output options for variable 'IPRT'
%   0 - only integrated results are printed to File smarts295.out.txt
%   1 - integrated and spectral results are printed to File smarts295.out.txt
%   2 - spectral results are printed to separate File smarts295.ext.txt
...
p = inputParser;
% CARD1
addParameter(p,'COMNT','default comment',@(x) ischar(x) && length(x)<=64)
% CARD2
...
addParameter(p,'ALTIT',0,@(x) isscalar(x) && x<=100) % ISPR 1 or 2
addParameter(p,'HEIGHT',0,@(x) isscalar(x) && x<=100) % ISPR 1 or 2
...
%CARD7a
addParameter(p,'ISPCTR',0,@(x) isscalar(x) && isnumeric(x) && x>=-1 && x<=8)
%CARD8 aerosol model
...
addParameter(p,'SOLARC',1367,@(x) isnumeric(x) && x>0)
...
%CARD12c
addParameter(p,'IOUT',[1 2 3 5 21],@(x) isnumeric(x) && all(x>=1) && all(x<=43))
...
switch CARD2.Value
    case 0
        ...
        card2a.Name = 'SPR';
    case 1
        card2a.Value = [p.Results.SPR p.Results.ALTIT p.Results.HEIGHT];
        ...
        assert(sum(card2a.Value(2:3))<=100,'''ALTIT''+''HEIGHT'' must be <=100')
        card2a.Name = 'SPR ALTIT HEIGHT';
    case 2
        card2a.Value = [p.Results.LATIT p.Results.ALTIT p.Results.HEIGHT];
        assert(sum(card2a.Values(2:3))<=100,'''ALTIT''+''HEIGHT'' must be <=100')
