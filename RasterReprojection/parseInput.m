...
if isempty(p.Results.InR)
    assert(isequal(size(inLat),sizeA) &&...
        ...
        'if input referencing matrix or raster reference is empty, lat/lon of same size as input raster must be specified')
    InRR = [];
    latlim = [min(inLat(:)) max(inLat(:))];
    lonlim = [min(inLon(:)) max(inLon(:))];
else
    assert(isempty(p.Results.latitude) && isempty(p.Results.longitude),...
        'if input referencing matrix or raster reference is specified, then input ''lat'' and ''lon'' must NOT be specified')
