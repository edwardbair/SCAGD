...
if iStruct.substance==snow && ~isempty(iStruct.fractionalCoverage)
    assert(length(iStruct.fractionalCoverage)==2 ||...
        length(iStruct.fractionalCoverage)==3,...
        'fractionalCoverage must have 2 or 3 values, corresponding to fSCA and 1 or 2 endmembers')
    ...
    % R0 same size as wavelength, or scalar, x # of non-snow endmembers
...
if isfield(iStruct,'corrFactor')
    if ~isempty(iStruct.corrFactor) && ~isempty(iStruct.cosZ)
        refl = refl.*iStruct.cosZ./iStruct.corrFactor;
    end
end
R.refl = refl;
if any(trans(:)>0)
